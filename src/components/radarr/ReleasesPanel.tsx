...
import { fetchReleases, grabRelease } from "@/hooks/useRadarr";
import type { RadarrRelease } from "@/types/radarr";

interface ReleasesPanelProps {
  movieId: number;
}

export function ReleasesPanel({ movieId }: ReleasesPanelProps) {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchReleases(movieId);
      setReleases(data);
    } catch {
      setReleases([]);
    } finally {
  ...
  const handleGrab = async (release: RadarrRelease) => {
    setGrabbing(release.guid);
    try {
      await grabRelease(release.guid, release.indexerId);
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {release.protocol === "torrent" ? (
                        <Badge variant="default">Torrent</Badge>
                      ) : (
