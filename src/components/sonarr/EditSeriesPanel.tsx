...
import { useSonarrProfiles, useSonarrRootFolders } from "@/hooks/useSonarr";
...
interface EditSeriesPanelProps {
  series: SonarrSeries;
  onSaved: () => void;
}

export function EditSeriesPanel({ series, onSaved }: EditSeriesPanelProps) {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/sonarr/series/${series.id}`, {
        method: "PUT",
  ...
  if (!open) {
    return (
      <button
        ...
        className="px-3 py-1.5 rounded-lg text-xs font-medium bg-white/[0.05] text-foreground border border-white/[0.08] hover:bg-white/[0.08] transition-colors"
      >
  ...
  return (
    <Card glow="cyan" className="mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        {/* Series Type */}
        <div>
          <label className="text-xs text-muted mb-1.5 block">Serientyp</label>
          <select
            className={inputClass}
            value={form.seriesType}
            onChange={(e) =>
              setForm({ ...form, seriesType: e.target.value })
            }
          >
        ...
        <div>
          ...
          <select
            className={inputClass}
            value={form.seasonFolder ? "true" : "false"}
            onChange={(e) =>
              setForm({ ...form, seasonFolder: e.target.value === "true" })
            }
          >
            ...
            <div className="flex flex-wrap gap-2">
              {rootFolders.map((rf) => (
                <button
                  ...
                  onClick={() => {
                    const seriesFolder =
                      series.path.split("/").pop() || series.title;
                    setForm({
                      ...form,
                      path: `${rf.path.replace(/\/$/, "")}/${seriesFolder}`,
                    });
                  }}
